...
public sealed class LayoutEngine
{
    ...
    public LayoutNode ComputeLayout(Template template)
    {
        ...
        var childIntrinsics = new List<IntrinsicSize>();
        ...
        var rootIntrinsic = childIntrinsics.Count > 0
            ? new IntrinsicSize(
                childIntrinsics.Max(c => c.MinWidth),
                childIntrinsics.Max(c => c.MaxWidth),
                childIntrinsics.Sum(c => c.MinHeight),
                childIntrinsics.Sum(c => c.MaxHeight))
            : new IntrinsicSize();
    ...
    private IntrinsicSize MeasureFlexIntrinsic(FlexElement flex, Dictionary<TemplateElement, IntrinsicSize> sizes)
    {
        ...
        var childSizes = new List<IntrinsicSize>();
        ...
        if (flex.Direction == FlexDirection.Column)
        {
            ...
            minWidth = childSizes.Max(c => c.MinWidth);
            maxWidth = childSizes.Max(c => c.MaxWidth);
            minHeight = childSizes.Sum(c => c.MinHeight) + totalGaps;
            maxHeight = childSizes.Sum(c => c.MaxHeight) + totalGaps;
        }
        else
        {
            // Row: width = sum of children + gaps, height = max of children
            minWidth = childSizes.Sum(c => c.MinWidth) + totalGaps;
            maxWidth = childSizes.Sum(c => c.MaxWidth) + totalGaps;
            minHeight = childSizes.Max(c => c.MinHeight);
            maxHeight = childSizes.Max(c => c.MaxHeight);
        }
    ...
    private bool MayWrapOrContainsNewlines(TextElement text, float contentWidth, LayoutContext context)
    {
    ...
    private LayoutNode LayoutQrElement(QrElement qr, LayoutContext context)
    {
    ...
    /// <param name="context">The layout context.</param>
    /// <returns>A layout node with computed dimensions.</returns>
    private LayoutNode LayoutBarcodeElement(BarcodeElement barcode, LayoutContext context)
    {
    ...
    /// <param name="context">The layout context.</param>
    /// <returns>A layout node with computed dimensions.</returns>
    private LayoutNode LayoutImageElement(ImageElement image, LayoutContext context)
    {
    ...
    private void LayoutColumnFlex(LayoutNode node, FlexElement flex, LayoutContext context, PaddingValues padding, float gap)
    {
        var crossAxisSize = node.Width - padding.Horizontal;

        // Calculate flex grow
        var totalGrow = node.Children.Sum(c => GetFlexGrow(c.Element));
        var totalChildHeight = node.Children.Sum(c => c.Height);
        ...
        if (freeSpace < 0 && totalGrow == 0 && node.Height > 0)
        {
            ...
            var totalShrinkBasis = node.Children.Sum(c => GetFlexShrink(c.Element) * c.Height);
    ...
    private void LayoutRowFlex(LayoutNode node, FlexElement flex, LayoutContext context, PaddingValues padding, float gap)
    {
        ...
        // Calculate flex grow
        var totalGrow = node.Children.Sum(c => GetFlexGrow(c.Element));
        var totalChildWidth = node.Children.Sum(c => c.Width);
        var totalGaps = gap * Math.Max(0, node.Children.Count - 1);
        ...
        {
            var overflow = -freeSpace;
            var totalShrinkBasis = node.Children.Sum(c => GetFlexShrink(c.Element) * c.Width);
            if (totalShrinkBasis > 0)
            {
                foreach (var child in node.Children)
                {
    ...
    private void StackChildrenVertically(LayoutNode parent, LayoutContext context)
    {
        var y = 0f;
    ...
    private float CalculateTotalHeight(LayoutNode node)
    {
        if (node.Children.Count == 0)
            return 0f;

        return node.Children.Max(c => c.Bottom);
    }
    ...
    private float CalculateTotalWidth(LayoutNode node)
    {
        if (node.Children.Count == 0)
            return 0f;

        return node.Children.Max(c => c.X + c.Width);
    }
}
