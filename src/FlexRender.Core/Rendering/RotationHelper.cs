...
public static class RotationHelper
{
    ...
    public static float ParseRotation(string? rotation)
    {
        ...
        // Handle named rotations (case-insensitive, allocation-free)
        if (string.Equals(rotation, "none", StringComparison.OrdinalIgnoreCase))
            return 0f;
        if (string.Equals(rotation, "right", StringComparison.OrdinalIgnoreCase))
            return 90f;
        if (string.Equals(rotation, "flip", StringComparison.OrdinalIgnoreCase))
            return 180f;
        if (string.Equals(rotation, "left", StringComparison.OrdinalIgnoreCase))
            return 270f;
