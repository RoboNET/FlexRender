...
/// </summary>
public sealed class ImageProvider : IContentProvider<ImageElement>
{
    ...
    public SKBitmap Generate(ImageElement element) => Generate(element, imageCache: null);
    ...
    /// <exception cref="FileNotFoundException">Thrown when the source file does not exist.</exception>
    /// <exception cref="InvalidOperationException">Thrown when the image cannot be decoded.</exception>
    public SKBitmap Generate(ImageElement element, IReadOnlyDictionary<string, SKBitmap>? imageCache)
    {
