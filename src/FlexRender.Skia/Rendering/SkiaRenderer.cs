...
public sealed class SkiaRenderer : IFlexRenderer, ILayoutRenderer<SKBitmap>
{
    ...
    private readonly ImageProvider _imageProvider = new();
    ...
    public LayoutNode ComputeLayout(Template template, ObjectValue data)
    {
        ...
        var processedTemplate = ProcessTemplate(template, data);
        ...
        IReadOnlyDictionary<string, SKBitmap>? imageCache)
    {
        ...
        switch (element)
        {
            ...
            case ImageElement image:
                using (var bitmap = _imageProvider.Generate(image, imageCache))
                {
                    canvas.DrawBitmap(bitmap, x, y);
                }
                break;
    ...
    private void RegisterTemplateFonts(Template template)
    {
        ...
        {
            _fontManager.RegisterFont(fontName, fontDef.Path, fontDef.Fallback);
            ...
            {
                _fontManager.RegisterFont("main", fontDef.Path, fontDef.Fallback);
            }
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var imageCache = _imageLoader is not null
            ? await PreloadImagesAsync(template, data, cancellationToken).ConfigureAwait(false)
            : null;

        try
        {
            // Measure returns the final size after rotation
            var size = Measure(template, data);
            var bitmap = new SKBitmap((int)size.Width, (int)size.Height);
            try
            {
                RenderToBitmapCore(bitmap, template, data, default, imageCache);
                return bitmap;
            }
            catch
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            RenderToBitmapCore(bitmap, template, data, offset, imageCache);
        }
        finally
        {
            if (imageCache is not null)
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        return Task.FromResult(Measure(template, data));
    }
