...
public static partial class YamlPreprocessor
{
    ...
    {
        // Use StringBuilder to avoid creating new strings on each Replace call
        var sb = new StringBuilder(content);

        // Replace @index, @first, @last
        sb.Replace("{{@index}}", index.ToString(CultureInfo.InvariantCulture));
        sb.Replace("{{@first}}", (index == 0).ToString().ToLowerInvariant());
        sb.Replace("{{@last}}", (index == count - 1).ToString().ToLowerInvariant());

        // Replace {{this}} with item value
        if (item is StringValue sv)
        {
            sb.Replace("{{this}}", sv.Value);
        }
