...
public sealed class SvgQrRenderingTests
{
    ...
    private static (SvgRenderingEngine Engine, TemplatePipeline Pipeline) CreateEngineWithSvgProvider(ISvgContentProvider<QrElement> svgProvider)
    {
    ...
    private static (SvgRenderingEngine Engine, TemplatePipeline Pipeline) CreateEngineWithRasterProvider(IContentProvider<QrElement> rasterProvider)
    {
        var limits = new ResourceLimits();
        var expander = new TemplateExpander(limits);
        var pipeline = new TemplatePipeline(expander, new TemplateProcessor(limits));
